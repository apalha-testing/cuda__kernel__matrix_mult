...
arguments = [D_kernel_double, A_double, B_double, n_columns_A, n_columns_D, n_rows_D]
...
arguments = [D_kernel_single, A_single, B_single, n_columns_A, n_columns_D, n_rows_D]
...
arguments = [D_kernel_single, A_single, B_single, n_columns_A, n_columns_B, n_rows_A]
...
# Setup kernel
kernel_name = "matrix_mult_tiling_optimization_2_double"
kernel_source = "../cu/matrix_mult_tiling_optimization_2_double.cu"
...
# Setup kernel
kernel_name = "matrix_mult_tiling_optimization_2_single"
kernel_source = "../cu/matrix_mult_tiling_optimization_2_single.cu"
...
metrics = OrderedDict()
metrics["GFLOPS/s"] = lambda p : (2 * n_columns_A * n_columns_D * n_rows_D / 1e9) / (p["time"] / 1e3)
...
# %% Reference Implementation from bvanwekhoven for square matrices
# Setup kernel
problem_size = (4096, 4096)
size = numpy.prod(problem_size)

A_square = numpy.random.randn(*problem_size).astype(numpy.float32)
B_square = numpy.random.randn(*problem_size).astype(numpy.float32)
C_square = numpy.zeros_like(A_square)

args = [C_square, A_square, B_square]
...
answer = [numpy.dot(A_square, B_square), None, None]
...
answer = [numpy.dot(A_single, B_single), None, None]
