/**
 * @file matrix_mult_tiling_optimization_1_double.cu
 * 
 * CUDA code to calculate D = A*B (tiling algorithm avoid bank conflict and additional optimization, single precision)
 * 
 */
...
/** Main entry point.
 * Computes matrix multiplication using optimized tiling algorithm (single precision)
 */
__global__ void matrix_mult_tiling_optimization_1_single( 
                      float * D, 
                      float * A, 
                      float * B, 
	...
	// Fill sub-array A_tile
	for (int k = 0; k < 4; k++){
	    A_tile[(k*4 + ty) + TILE_SIZE * tx] = A[a_idx + w_A_h_B * (k*4 + ty) + tx];  // transposes A into A_tile
	}
    ...
    int d_idx = w_B * TILE_SIZE * by + VECTOR_SIZE * TILE_SIZE * bx;  // the blocks in x (columns) go over VECTOR_SIZE tiles but the blocks in y go over one tile
    d_idx += TILE_SIZE * ty + tx;  // the treads in each go over consecutive columns and the ones in y jump a TILE_SIZE
